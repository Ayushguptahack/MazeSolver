...
wn.setup(1300,700)
print("\t\t\t####################### Welcome to A* maze solver programme.#######################\n\n");
print("There are 5 maze designes in this programme.\n")
a=int(input("Please Select a grid by Entering number 1,2,3,4,5:- "))
if a==1:
    grid = [
            "+++++++++++++++",
            "+s+       + +e+",
    ...
    while len(openlist) > 0:
        ...
        for i in range(len(neighbourslist)):
            ...
            if neighbour not in closelist and not neighbour.wall:
                time.sleep(0)
                green.goto(-588 + (neighbour.j * 21), 288 - (neighbour.i * 21))
                green.stamp()
                tempg = current.g+1
                newpath=False
